...
void counterspell(Spell *spell) {
   ...
   if (Fireball *s = dynamic_cast<Fireball *>(spell))
    ...
    } else
    {
        ...
        vector<vector<size_t> > matrix(name.size() + 1, vector<size_t>(journal.size() + 1, 0));
        for (size_t i = 0; i < name.size(); ++i) {
            for (size_t j = 0; j < journal.size(); ++j) {
               
                if (name[i] == journal[j]){
                    // Extend both axes of matrix with the total of matches.
        ...
        cout << matrix[name.size()][journal.size()] << endl;
